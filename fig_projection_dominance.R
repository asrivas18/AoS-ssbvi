...
# Evaluator Function
# -----------------
evaluate_projection_dominance <- function(n, nu, M, cluster) {
...
out_list <- lapply(nu_grid, function(nu) {
  evaluate_projection_dominance(n = n, nu = nu, M = M, cluster = cl)
})
...
# Save outputs
# -----------------
ggsave("results/fig_projection_dominance.pdf", plot = p, width = 8.5, height = 5.8, dpi = 300)
ggsave("results/fig_projection_dominance.png",  plot = p, width = 8.5, height = 5.8, dpi = 320)

write.csv(res_rn, "results/fig_projection_dominance_raw.csv", row.names = FALSE)

# Print summary
cat("\n=== Projection Dominance Diagnostic Complete ===\n")
print(res_rn[, c("nu", "M_valid", "success_rate", "R_n")], digits = 4)

# Save session info for reproducibility
writeLines(capture.output(sessionInfo()), "results/fig_projection_dominance_sessionInfo.txt")
